  ...
  Modal.prototype = {
    ...
    show: function () {
      var that = this,
        e = $.Event('show'),
        body = $('body'),
        openModals = parseInt(body.attr('data-open-modals'))
        this.$element.trigger(e)
        if (this.isShown || e.isDefaultPrevented()) return
        if (_.isNaN(openModals) || openModals <= 0) {
          openModals = 0
          $('body').addClass('modal-open')
        }
      this.modalNumber = openModals + 1
      body.attr('data-open-modals', this.modalNumber)
      this.isShown = true
      escape.call(this)
    ...
    hide: function (e) {
      ...
      var that = this,
        ...
        this.isShown = false
      if (openModals > 0) {
        body.attr('data-open-modals', openModals - 1)
        if (openModals == 1) {
  ...
  function escape() {
    ...
    if (this.isShown && this.options.keyboard) {
      $(document).on('keyup.dismiss.modal', function (e) {
        if (e.which == 27) {
          var openModals = parseInt($('body').attr('data-open-modals'))
          if (openModals != that.modalNumber) {
            return
          }
          that.hide()
        }
